# APM Slash Commands Manager for Claude Code (Windows PowerShell Version)
# This script installs and uninstalls Claude Code slash commands from the APM prompts directory
...
# Define the Claude Code commands directory
...
function Update-Commands {
    ...
    if ($DryRun) {
        ...
        Write-Host "Claude Code Commands Directory: $ClaudeCommandsPath" -ForegroundColor Yellow
        
        # Check if directory exists and find existing commands
        if (Test-Path $ClaudeCommandsPath) {
            $ExistingCommands = Get-ChildItem -Path $ClaudeCommandsPath -Filter "apm-*.md" -ErrorAction SilentlyContinue
            if ($ExistingCommands) {
            ...
            Write-Host "Claude Code commands directory does not exist" -ForegroundColor Yellow
        }
    ...
    if (-not (Test-Path $ClaudeCommandsPath)) {
        if ($DryRun) {
            Write-Host "[DRY RUN] Would create directory: $ClaudeCommandsPath" -ForegroundColor Yellow
        }
        else {
            New-Item -ItemType Directory -Path $ClaudeCommandsPath -Force | Out-Null
            Write-Host "Created Claude Code commands directory: $ClaudeCommandsPath" -ForegroundColor Green
        }
    }
    ...
    foreach ($File in $MarkdownFiles) {
        ...
        $CommandFilePath = Join-Path $ClaudeCommandsPath "${CommandNameWithPriority}.md"
        
        # Read the original file content with UTF-8 encoding
        $Content = Get-Content $File.FullName -Raw -Encoding UTF8
        
        # Add metadata header for the Claude Code command
        $ClaudeCommandContent = @"
---
...
"@
        ...
        if ($DryRun) {
            ...
            [System.IO.File]::WriteAllText($CommandFilePath, $ClaudeCommandContent, $Utf8NoBom)
...
function Uninstall-Commands {
    ...
    if (-not (Test-Path $ClaudeCommandsPath)) {
        Write-Host "Claude Code commands directory does not exist: $ClaudeCommandsPath" -ForegroundColor Yellow
...
"@
    ...
    Write-Host ""
    Write-Host "To use these commands in Claude Code:" -ForegroundColor Cyan
