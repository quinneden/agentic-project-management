...
# APM Slash Commands Manager for Claude Code (Mac/Unix Version)
...
update_commands() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        echo -e "${YELLOW}Claude Code Commands Directory: $CLAUDE_COMMANDS_PATH${NC}"

        # Check if directory exists and find existing commands
        if [[ -d "$CLAUDE_COMMANDS_PATH" ]]; then
            EXISTING_COMMANDS=($(find "$CLAUDE_COMMANDS_PATH" -name "apm-*.md" 2>/dev/null || true))
            ...
            echo -e "${YELLOW}Claude Code commands directory does not exist${NC}"
        fi
    ...
    if [[ ! -d "$CLAUDE_COMMANDS_PATH" ]]; then
        if [[ "$DRY_RUN" == "true" ]]; then
            echo -e "${YELLOW}[DRY RUN] Would create directory: $CLAUDE_COMMANDS_PATH${NC}"
        else
            mkdir -p "$CLAUDE_COMMANDS_PATH"
            echo -e "${GREEN}Created Claude Code commands directory: $CLAUDE_COMMANDS_PATH${NC}"
        fi
    fi
    ...
    while IFS= read -r -d '' file; do
        ...
        COMMAND_FILE_PATH="$CLAUDE_COMMANDS_PATH/${COMMAND_NAME_WITH_PRIORITY}.md"
...
$CONTENT"
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            echo "$CLAUDE_COMMAND_CONTENT" > "$COMMAND_FILE_PATH"
...
uninstall_commands() {
    ...
    if [[ ! -d "$CLAUDE_COMMANDS_PATH" ]]; then
        echo -e "${YELLOW}Claude Code commands directory does not exist: $CLAUDE_COMMANDS_PATH${NC}"
...
$CONTENT"
    ...
    echo ""
    echo -e "${CYAN}To use these commands in Claude Code:${NC}"
    echo -e "${WHITE}  1. Open Claude Code CLI${NC}"
